...
impl QcPager {
    ...
    pub fn save(&mut self, k: u32, v: String) -> Option<usize> {
        ...
        if vlen > 0 {
            ...
            let m_contain = &mut self.0[mstar..=(data_pointer as usize)];
    ...
    pub fn obtain(&self, k: u32) -> Option<String> {
        ...
        return Some(String::from_utf8(self.0[pointer..(pointer + len)].to_vec()).unwrap());
    }
    ...
    pub fn report(&self) {
        ...
        let slot_list: Vec<(u32, u16, u16, String)> = self.0[slot_start..(slot_start+slot_len)]
            ...
            .map(|w| (w.0, w.1, w.2, String::from_utf8(self.0[(w.1 as usize)..(w.1 + w.2) as usize].to_vec()).unwrap()))
            .collect();
    ...
    fn binary_search(&self, key: u32) -> (usize, Option<[u8;Self::SLOT_SIZE]>) {
        ...
        let block_list: Vec<u32> = (slot_start..(slot_start+slot_len))
            ...
            .map(|op| {
                u32::from_be_bytes(self.0[op..(op+4)].try_into().unwrap())
            })
            .collect();
        ...
        let mut pl = 0;
        let mut pr = if block_list.len() > 0 { block_list.len() - 1 } else { 0 };
        let mut out = (pl, None);
    ...
    fn set_slot_pointer(&mut self, pointer: u16) {
        [self.0[10], self.0[11]] = u16::to_be_bytes(pointer);
    }
    ...
    fn idx_slot(&self, idx: usize) -> Option<[u8;Self::SLOT_SIZE]> {
        ...
        out.copy_from_slice(&self.0[slot_start..(slot_start + Self::SLOT_SIZE)]);
    ...
    fn insert_slot(&mut self, slot: [u8; Self::SLOT_SIZE], idx: usize) -> Option<()> {
        ...
        // byte array ->> slot array
        let mut slot_list: Vec<[u8; Self::SLOT_SIZE]> = self.0[slot_start..(slot_start+slot_len)]
            .chunks(Self::SLOT_SIZE)
            .map(|v| <[u8; Self::SLOT_SIZE]>::try_from(v).unwrap())
            .collect();
        ...
        self.0[slot_start..(slot_start + slot_len + Self::SLOT_SIZE)].as_mut().clone_from_slice(&slot_list.concat());
    ...
    pub fn left_space(&self) -> u16 {
        self.get_data_pointer()
            - self.get_slot_pointer()
            - self.get_slot_len()
    }
}
